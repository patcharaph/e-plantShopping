...
function ProductList({ onHomeClick }) {
  ...
  // ✅ Task 1: addedToCart state
  const [addedToCart, setAddedToCart] = useState({});

  // (Optional) ใช้ทำ badge จำนวนบนไอคอน cart (Task 4/ต่อยอด)
  const CartItems = useSelector((state) => state.cart.items);
  const calculateTotalQuantity = () => {
    return CartItems ? CartItems.reduce((total, item) => total + item.quantity, 0) : 0;
  };

  // ✅ Task 1: handleAddToCart
  ...
  const styleObj = {
    ...
    color: "#fff", // ✅ !important ใช้ไม่ได้ใน inline style
    padding: "15px",
    display: "flex",
    justifyContent: "space-between",
    alignItems: "center", // ✅ แก้สะกด
    fontSize: "20px",
  };
  ...
  return (
    <div>
      ...
      {!showCart ? (
        // ✅ Task 1: render plants inside product-grid
